...
from src.vision_module import ObjectDetector
from src.caption_module import SceneCaptioner, SpatialReasoningNLP

app = FastAPI(title="Blind-Project API")
...
def load_models():
    ...
    print("Loading YOLO and mBLIP models...")
...
def background_caption_task(frame: np.ndarray):
    """Runs mBLIP captioning in the background. It will return early if interval hasn't passed."""
...
async def analyze_frame(request: FrameRequest, background_tasks: BackgroundTasks):
    try:
        ...
        # Trigger background mBLIP captioning
